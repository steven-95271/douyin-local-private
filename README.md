...
内部自用的本地内容同步工具：把抖音、微博、X、小宇宙、公众号和 YouTube 内容整理成 Obsidian Markdown，并支持日报/周报。

稳定入口是本地网页 Dashboard。

## 当前能力

- 内容源库：维护抖音、微博、X、小宇宙、公众号、YouTube 等来源，并按平台和分类折叠展示。
- URL 补全：用本地脚本自动读取昵称、简介、分类、标签和平台内部 ID，不调用大模型。
- 抓取入库：抖音视频转录、微博正文、X 推文、小宇宙播客转录、公众号公开文章解析、YouTube 字幕/音频转录。
- AI 笔记：普通摘要默认使用 `glm-5-turbo`，长播客默认使用 `glm-5.2`，外文 X/YouTube 会整理成中文并保留原文逐字稿；也可只保存原文或逐字稿。
- 运行记录：查看嗅探数量、成功、等待、失败原因，并支持失败项重爬。
- 候选抓取：先嗅探候选内容，再手动勾选需要抓取的条目。
- 抓取前过滤：下载/转录/AI 总结前跳过商单、导流、直播预告、唱歌跳舞、随拍打卡等低价值内容。
...
| YouTube | 字幕优先，必要时音频转录；外文默认中文整理 | `YouTube/视频博主` |

小红书因账号风控风险已暂停接入，不参与自动抓取和定时任务。B站、TikTok、快手、贴吧、知乎在前端保留为内容源档案，抓取适配器后续再接入。
...
YouTube 抓取依赖 `yt-dlp` Python 包。更新旧环境后建议执行一次：
...
抖音建议使用 Dashboard「账号与模型」里的「扫码登录」。系统会打开真实 Chrome 窗口并使用本地浏览器 profile 保存登录态；登录成功后会自动同步当前会话给抓取流程。首次使用前如果提示 Playwright 缺失，运行：
...
- 在「内容源库」添加主页 URL，然后点击「URL 补全」。X 可填 `https://x.com/username` 或 `@username`；公众号来源可以直接填公众号名称；YouTube 可填频道 URL 或视频 URL；公众号需要先登录 `mp.weixin.qq.com` 后台并用插件导入公众号后台 Cookie。
- 在「抓取中心」选择内容源。
...
- 「过滤低价值内容」会在下载前用标题、描述和话题标签跳过明显非口播或低信息密度内容；已过滤内容会写入状态库，后续自动任务不会反复处理。
- 失败项可以在运行记录中一键重爬。
...
YouTube 支持频道 URL 或视频 URL。系统优先读取官方/自动字幕；没有字幕时下载音频并用 Whisper 转录。中文频道按普通视频整理；英文等非中文频道默认输出中文摘要和结构化笔记，同时在 Markdown 中保留原文逐字稿。
...
自检显示需要处理：优先看 Dashboard 的「自检」面板。网络中断、请求超时、AI 总结失败会自动重试；如果提示微博/公众号/抖音登录态失效，需要重新登录并用插件或扫码方式更新登录态，然后在运行记录里重爬失败项。
