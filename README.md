...
基于 `Evil0ctal/Douyin_TikTok_Download_API` 做了本地封装。当前可抓取平台是 Douyin；微博、YouTube、B站、TikTok 已在前端预留为内容源档案，抓取适配器后续接入。请仅用于自己有权访问和整理的内容。

## 功能

- 维护本地「内容源库」。
- 通过 Chrome 插件导入抖音 Cookie。
- 自动补全抖音来源的昵称、简介、分类、标签和内部 ID。
- 抓取启用抖音来源的新视频，已成功处理过的视频会跳过。
...
- 启动单个可抓取来源。
- 一键串行跑所有可抓取且启用的来源。
- 查看最近一次任务进度、失败/等待/进行中的视频、失败原因和输出文件。
