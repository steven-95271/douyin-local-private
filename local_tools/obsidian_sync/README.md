...
内部自用工具：把已配置抖音口播博主的视频、微博博主的文字内容、X 推文、小宇宙播客单集和公众号文章自动转成 Obsidian Markdown 笔记。

当前可抓取平台是抖音、微博、X、小宇宙和公众号。小红书因账号风控风险已暂停接入；YouTube、B站、TikTok、快手、贴吧、知乎已在 Dashboard 里预留为内容源档案，抓取适配器后续接入。
...
- 抖音建议使用扫码登录；微博/公众号后台仍通过 Cookie 或 token 接入。
...
- 「过滤低价值内容」会在下载、OCR、Whisper 和 AI 总结前，用标题、描述、话题标签跳过明显商单、导流、直播预告、唱歌跳舞、随拍打卡等内容；已过滤内容会写入状态库，后续自动任务直接跳过。
...
目前抖音视频抓取、微博文本抓取、X 推文抓取、小宇宙公开播客抓取和公众号公开文章抓取已接入；小红书已暂停接入，YouTube、B站、TikTok、快手、贴吧、知乎下一阶段接入。
...
抖音建议在 Dashboard「账号与模型」里使用「扫码登录」。系统会打开真实 Chrome 窗口，使用本地 browser profile 保存登录态，并在登录成功后把当前会话同步给抓取流程。
