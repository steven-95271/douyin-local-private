...
{
  echo
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] DAILY BRIEF START"
  .venv/bin/python local_tools/obsidian_sync/weekly_brief.py --config "$config_file" --period daily
  echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] DAILY BRIEF DONE"
} >> "$log_file" 2>&1
